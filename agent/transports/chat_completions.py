    ...
    def build_kwargs(
        ...
        **params,
        ...
        if tools:
            ...
            # Gated on ``if tools`` so requests without tools don't send
            # tool_choice=auto — some providers (vLLM without tools configured)
            # reject the request. (Issue A, PR #34)
        ...
        # tool_choice="required" and override reasoning_effort=low for this
        # single retry. reasoning_effort=none breaks K3 structural tag emission,
        # so we force a non-none value only on the stall retry.
        try:
            ...
            if latest_user_message_is_stall_continuation(sanitized):
                ...
                _existing_effort = api_kwargs.get("reasoning_effort", "")
                if _existing_effort not in ("low", "medium", "high"):
